...
├── .github/
│   └── workflows/
│       └── monitor.yml           # GitHub Actions 定时任务（UTC 10:00 = 北京 18:00）
├── config/
...
| 期权 Greeks | Black-Scholes + yfinance IV | 自行计算，无需 Tradier |
| IV 来源 | yfinance 期权链 IV（中位数）× 70% + HV20 × 30% | 混合后 Delta 误差 < 0.5% |
...
原因：yfinance 对远月深度实值 LEAPS 返回的 IV 偏高约 6%，
     导致价格高估、Delta 低估。混合后：
     - Delta 误差 < 0.5%（620C 实测：模型 0.810 vs 市场 0.8119）
     - 价格误差从 8% 降至 4–5%
...
**总资产基准**：$100,000  
**现金**：$66,305（现金占比 66.3%）  
**历史期权总投入基准**：$20,352（三张合约建仓成本之和）

| 合约 ID | 行权价 | 到期日 | 张数 | 买入均价 | DTE | 豁免续杯 |
|---------|--------|--------|------|---------|-----|---------|
| QQQ_261218_620C | $620 | 2026-12-18 | 1 | $63.00 | 208 | ✅ 是 |
| QQQ_261218_630C | $630 | 2026-12-18 | 1 | $64.29 | 208 | ✅ 是 |
| QQQ_270331_705C | $705 | 2027-03-31 | 1 | $76.23 | 311 | — |
...
- [x] **GitHub Actions 部署**：UTC 10:00 自动运行，DBs 通过 cache 持久化
...
- [x] **豁免续杯**：exempt_rollout 字段，620C / 630C 跳过 ROLL_OUT 仅等待 HARVEST
- [x] **历史价格数据库**：history_store.py + backfill_history.py，5 年 QQQ 日线入库
